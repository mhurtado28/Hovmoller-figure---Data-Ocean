...
opts.Sheet = "Hoja2";
opts.DataRange = "Q2:S2174";

% Specify column names and types
opts.VariableNames = ["fechanum", "Fecha", "Valor"];
...
% Import the data
tbl = readtable("C:\CursoSerieTiempo\tarea1\lluvias_tayrona.xlsx", opts, "UseExcel", false);

%% Convert to output type
fechanum = tbl.fechanum;
FechaTay = tbl.Fecha;
ValorTay = tbl.Valor;
...
FechaTay = datevec(FechaTay)

%HOMWOLLER Reorganiza la serie de tiempo
yyt = unique(FechaTay(:,1));
mmt = 1 : 13;
nyt = length(yyt);

STt = NaN(nyt,13);
for kt = 1 : nyt
    iyt = find( FechaTay(:,1)==yyt(kt) );
    mt  = FechaTay(iyt,2);
    STt(kt,mt) = ValorTay(iyt);
end
...
set(gca,"xtick",mmt+0.5, "xticklabel",mes,"Linewidth",2,"fontsize",12,...
        "xlim",[1 13],"ytick",[2000:1:2010],"ylim",[2000,2010])
